...
2. **Выпускник → сначала email (второй авто-матч).** Бот просит ввести email
   с явной подсказкой, что нужен **основной e-mail из профиля на my.nes.ru**
   (в директории у человека ровно один e-mail — текущий). Дефолтный текст
   (редактируемый шаблон `on_email_prompt_message`, как приветствие):
   > 📧 Введите ваш основной e-mail, указанный в профиле на my.nes.ru (раздел
   > «Контактная информация»). Сверим его с директорией — если найдём, сразу
   > вас узнаем. Не помните или не совпадёт — ничего страшного, продолжим вручную.

   Введённое → `declared_email` (нормализ. lower). Ищем `alumni_person`, где
   `email ∈ emails`.
...
3. **Год** (для выпускника/студента): десятилетие → год. Список годов —
   ...
   - **Студент** («ожидаемый выпуск»): годы из базы **плюс будущие**:
     ...
     Год обязателен (кнопки «пропустить» нет).
...
- `chats.on_email_prompt_message` (новый) — запрос почты на ветке «Я выпускник».
  Дефолт: см. §2 (явно просит основной e-mail из профиля my.nes.ru).
